...
Lena uses Git for source control. The main branch is `main`. Changes **must** be done via pull
requests. Commits _should_ be granular but logical – a large enhancement/fix should be broken up
into multiple commits that represent steps or progression toward the whole.

Commit messages **must** be done using the Conventional Commit style, with a maximum header line
of 72 characters, and maximum body lines of 80 characters. A single summary line is fine
for simple changes, but larger or more complex changes _should_ include more details in the
body. When the issue number is known, a `Refs: #1` or `Fixes: #1` trailer **must** be used,
depending on whether the commit completely resolves the issue/implements the feature. The
agent **must** also include a `Co-authored-by: Agent <agent@provider.ai>` trailer.
